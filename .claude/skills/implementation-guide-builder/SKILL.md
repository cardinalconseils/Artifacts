...
description: "Transforms any input — a video, article, framework, product, raw idea, or shared link — into a structured implementation guide PMC can use himself, sell to clients, package as a product, or teach as a framework. Asks up front: venture (Cardinal Conseils, PayFacto, ServiConnect, Proposai, personal), format (md, interactive HTML, simple HTML, JSON, docx), Cardinal branding yes/no, and whether to publish to the cardinalconseils/artifact GitHub repo with GitHub Pages live URL and deployment monitoring. Saves to Obsidian, indexes in Brain, optionally publishes to GitHub. ALWAYS trigger when PMC says — build me a guide, turn this into a guide, make this sellable, package this, publish this, push to GitHub, deploy this artifact, implementation guide, playbook, framework, I want to teach this, I want to sell this — or when PMC shares content and asks for structure, packaging, extraction, or publishing. Also trigger when converting raw knowledge into a deployable, monetizable, shareable asset."
---
...
Push to the `cardinalconseils/artifact` GitHub repo? This is the public/shareable layer on top of Obsidian. Obsidian is the private vault; GitHub is the publication channel with a live URL via GitHub Pages.
...
live_url: [GitHub Pages URL if HTML format was published, else null]
created: [YYYY-MM-DD]
...
### Target repo
```
cardinalconseils/artifact
```

GitHub Pages must be enabled on this repo (one-time manual setup, source = `main` branch, root). The skill assumes this is already done. If a push succeeds but the live URL never resolves, surface this to PMC as the likely cause.
...
**Use `mcp__github__push_files` to commit all files in a single commit.**
...
```json
{
  ...
  "version": 1,
  "versions": [],
  "github_url": "https://github.com/cardinalconseils/artifact/tree/main/[path]",
  "live_url": "https://cardinalconseils.github.io/artifact/[path]/",
  "tags": ["..."]
}
```

`live_url` is only meaningful when `index.html` is in the folder. If no HTML was produced, omit or null it.
...
After the commit lands, surface deployment status — not just "pushed, hope it works."

1. Wait ~5 seconds, then poll the GitHub Pages build status via the API: `GET /repos/cardinalconseils/artifact/pages/builds/latest`. Use `mcp__github__*` if exposed; otherwise fall back to step 3.
2. Three outcomes to report:
   - **`building`:** Tell PMC the push landed and Pages is building. Show both the GitHub URL (works immediately) and the future live URL (will resolve in ~30–60s).
   - **`built`:** Tell PMC it's live. Show both URLs as clickable.
   - **`errored`:** Tell PMC the push landed but Pages failed. Show the error message and the GitHub Actions link so he can debug.
3. If polling is not available, fall back to: surface the GitHub URL (always works) and the expected live URL (will resolve once Pages finishes), and tell PMC "Pages build typically completes within 60 seconds."

### Final confirmation line (when published)

> *Saved to vault as `[obsidian-path]` + indexed in Brain.*
> *Published: `https://github.com/cardinalconseils/artifact/tree/main/[repo-path]`*
> *Live: `https://cardinalconseils.github.io/artifact/[repo-path]/` ([building|live|errored])*

### Failure modes to handle

- **Push fails (auth, network, repo not found):** Surface the error and ask PMC to reconnect the GitHub MCP. The Obsidian save is already done — that's not lost.
...
- **Pages disabled on the repo:** Push still succeeds, but `live_url` won't resolve. Tell PMC once, link to the Pages settings page.
...
10. **If `publish` was selected, did the GitHub push land AND were both URLs surfaced?** A publish without confirmed URLs is a half-done publish.
